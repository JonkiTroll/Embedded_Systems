...
int main(int argc, char *argv[])
{
    ...
    file = open_and_set_fid(MSG_BASE, &old_options);  // Function that opens the file, and sets the options for raw mode and the desired speed.
    ...
    uint8_t receive[MSG_BASE];

   if (read_slave_feedback(file, receive, MSG_BASE)) printf("Some feedback error.\n");
...
int read_slave_feedback(int fid, uint8_t receive[], int len)
{
   ...
   if ((count = read(fid, (void *)receive, len)) < 0)
    ...
    {
      print_string_hex(receive, len, false); 
    }
    return 0;
}
